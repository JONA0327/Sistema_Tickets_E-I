...
<nav {{ $attributes->merge([
    ...
    <a href="{{ route('welcome') }}" class="{{ $linkBase }} gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="h-4 w-4 shrink-0">
    ...
    @if (auth()->check() && auth()->user()->isAdmin())
        <a href="{{ route('admin.dashboard') }}" class="{{ $linkBase }} gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="h-4 w-4 shrink-0">
    ...
    <a href="{{ route('tickets.mis-tickets') }}" class="{{ $linkBase }} gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="h-4 w-4 shrink-0">
    ...
    @if (auth()->check() && auth()->user()->isAdmin())
        <a href="{{ route('archivo-problemas.index') }}" class="{{ $linkBase }} gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="h-4 w-4 shrink-0">
